  ...
  const fetchReviewData = async () => {
    try {
      ...
      for (const review of reviewData) {
        ...
        movieDetails[movie_id] = data;
      }
      setMovie(movieDetails);
      const myReviews = reviewData.filter(
        (review) => review.username === profile.username
      );
      ...
      {profileEdit ? (
        ...
        <React.Fragment>
          ...
          <div className='reviews-heading-bg'>
            ...
            <h1 className='text-light text-center reviews-heading'>
              My Reviews
            </h1>
            <svg
              xmlns='http://www.w3.org/2000/svg'
          ...
          <div className='reviews-container'>
            <div className='liked-left scroll'>
              {reviews !== null ? (
                reviews.length > 0 ? (
                  reviews
                    .filter((review) => review.rating === true)
                    .map((review) => (
                      <div className='left-container' key={review.id}>
                            ...
                            <Link to={`/movie/${review.movie_id}`}>
                              <img
                                ...
                                src={`https://image.tmdb.org/t/p/w200${
                                  movie[review.movie_id].poster_path
                                }`}
                                alt={movie[review.movie_id].title}
                              />
                            </Link>
                          </div>
                        )}
                        <div className='review-text-container'>
                          <h4 className='text-light text-decoration-underline'>
                            {movie[review.movie_id].title}
                          </h4>
                          <h5 className='text-light'>{review.title}</h5>
                          <p className='text-light'>{review.body}</p>
                          <p className='text-light review-date'>
                            {new Date(review.posted_time).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    ))
            ...
            <div className='disliked-right scroll'>
              {reviews !== null ? (
                reviews.length > 0 ? (
                  reviews
                    .filter((review) => review.rating === false)
                    .map((review) => (
                      <div className='right-container' key={review.id}>
                        {movie[review.movie_id] && (
                          <div className='review-poster-container'>
                            <Link to={`/movie/${review.movie_id}`}>
                              <img
